...
class App:
    def __init__(self, master, calibrea, files = []):
        ...
        # max values here
        max_1 = Entry(master, fg = "purple", justify = "center")
        max_1.insert(END, "")
        max_1.grid(row = 7, column = 7)

        max_2 = Entry(master, fg = "purple", justify = "center")
        max_2.insert(END, "")
        max_2.grid(row = 8, column = 7)

        max_3 = Entry(master, fg = "purple", justify = "center")
        max_3.insert(END, "")
        max_3.grid(row = 9, column = 7)

        max_4 = Entry(master, fg = "purple", justify = "center")
        max_4.insert(END, "")
        max_4.grid(row = 10, column = 7)

        max_5 = Entry(master, fg = "purple", justify = "center")
        max_5.insert(END, "")
        max_5.grid(row = 11, column = 7)

        energy_1 = StringVar()
        energy_1.set('                        ') # this determines the width of frame_4
        option1 = OptionMenu(master, energy_1, *energy_dict.keys())
        option1.grid(row = 7, column = 8)

        energy_2 = StringVar()
        energy_2.set('                        ')
        option2 = OptionMenu(master, energy_2, *energy_dict.keys())
        option2.grid(row = 8, column = 8)

        energy_3 = StringVar()
        energy_3.set('                        ')
        option3 = OptionMenu(master, energy_3, *energy_dict.keys())
        option3.grid(row = 9, column = 8)

        energy_4 = StringVar()
        energy_4.set('                        ')
        option4 = OptionMenu(master, energy_4, *energy_dict.keys())
        option4.grid(row = 10, column = 8)

        energy_5 = StringVar()
        energy_5.set('                        ')
        option5 = OptionMenu(master, energy_5, *energy_dict.keys())
        option5.grid(row = 11, column = 8)

        self.calculateButton = Button(master, text='add to file', command= lambda: meth.add_to_file(self, max_1, max_2, max_3, max_4, max_5, energy_1, energy_2, energy_3, energy_4, energy_5, energy_dict, coefficient)) # resfresh if files are added or removed
        ...
        self.refreshButton = Button(master, text='refresh', command= lambda: meth.refreshList(self, calibrea)) # resfresh if files are added or removed
        self.refreshButton.grid(row = 0, column = 0)

        self.listbox = Listbox(master, height = 34)
        self.listbox.grid(row = 1, column = 0, rowspan = 34)

        scrollbar = Scrollbar(command = self.listbox.yview, orient=VERTICAL)
        scrollbar.grid(row=1, column = 1, rowspan = 34, sticky = 'ns')
        self.listbox.configure(yscrollcommand = scrollbar.set)

        self.listbox.bind("<Double-Button-1>", lambda event: meth.plot_update(event, self, canvas, a, f, max_1, max_2, max_3, max_4, max_5, energy_1, energy_2, energy_3, energy_4, energy_5))
   ...
   # adds buttons to listbox in alphabetical then numerical order
    def placeButtons(self, fileNames):
        ...
        for item in names:
            self.listbox.insert(END,item.split('/')[2])

root = Tk() # initialise new window
